...
# Extrair as poses: frente, virado para direita e virado para esquerda
        poses = ['frente', 'direita', 'esquerda']
        images = []

        for pose in poses:
            messagebox.showinfo("Registrar Face", f"Posicione o rosto virado para {pose} e clique em OK.")
            ret, frame = cap.read()
            ...
            if encoding:
                ...
                # Salvar a imagem em uma pasta com o nome do usuário e a pose
                id_usuario = gerar_id()
                folder = "faces"
                if not os.path.exists(folder):
                    os.makedirs(folder)
                path = f"{folder}/{id_usuario}/{nome}_{pose}.jpg"
                ...
                messagebox.showwarning("Erro", f"Nenhuma face detectada ou mais de uma face detectada na pose {pose}.")

        if len(images) == len(poses):
            messagebox.showinfo("Sucesso", "Faces registradas com sucesso!")
            # Conectar ao banco de dados
            conn = sqlite3.connect('Registros.db')
            c = conn.cursor()
            # Inserir os dados na tabela
            c.execute('INSERT INTO Registros (nome, sobrenome, data_nascimento, foto_path) VALUES (?, ?, ?, ?)',
                      (nome, sobrenome, data_nascimento, f"{folder}/{id_usuario}"))
            conn.commit()
            conn.close()
        else:
            messagebox.showwarning("Erro", "Não foi possível registrar todas as faces.")
    ...
    # Botão para registrar a face em diferentes poses
...
window.after(10, update_treeview)
